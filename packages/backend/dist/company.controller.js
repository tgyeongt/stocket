...
class CompanyController {
    ...
    async getDetail(req, res) {
        const parsed = company_dto_1.GetCompanyRequestDto.safeParse(req.params);
        if (!parsed.success) {
            res
                .status(400)
    ...
    async search(req, res) {
        const parsed = company_dto_1.SearchCompanyRequestDto.safeParse(req.query);
        if (!parsed.success) {
            res
                .status(400)
                .json({ success: false, errors: parsed.error.flatten().fieldErrors });
    ...
    async getSimilar(req, res) {
        const parsed = company_dto_1.GetSimilarCompaniesRequestDto.safeParse({
    ...
    async compare(req, res) {
        const parsed = company_dto_1.CompareCompaniesRequestDto.safeParse(req.body);
        if (!parsed.success) {
            res
                .status(400)
                .json({ success: false, errors: parsed.error.flatten().fieldErrors });
