...
const bigintToStr = (v) => v != null ? v.toString() : null;
// 0–100 범위로 클램프
const clamp = (v) => Math.min(100, Math.max(0, Math.round(v)));
// ── 성장성 점수 계산 헬퍼 ─────────────────────────────────────
function calcGrowth(revenueGrowthRate) {
    if (revenueGrowthRate == null)
        return 50;
    return clamp(50 + revenueGrowthRate * 2);
}
function calcStability(debtRatio) {
    if (debtRatio == null)
        return 50;
    // 부채비율이 낮을수록 높은 점수
    if (debtRatio <= 30)
        return 90;
    if (debtRatio <= 80)
        return clamp(90 - ((debtRatio - 30) / 50) * 20);
    if (debtRatio <= 150)
        return clamp(70 - ((debtRatio - 80) / 70) * 20);
    if (debtRatio <= 300)
        return clamp(50 - ((debtRatio - 150) / 150) * 25);
    return clamp(25 - ((debtRatio - 300) / 200) * 20);
}
function calcProfitability(roe, operatingMargin) {
...
function calcMomentum(m1, m3, m6) {
...
function resolveIndustryName(indutyCode, indutyName, sector) {
    if (sector)
        return sector;
    if (indutyName)
        return indutyName;
    if (indutyCode)
        return INDUSTRY_CODE_MAP[indutyCode.slice(0, 3)] ?? "기타";
    return "기타";
}
function calcOverallScore(axes) {
    return clamp(axes.growth * 0.35 +
        axes.stability * 0.25 +
...
// ── Why 카드 생성 ─────────────────────────────────────────────
function generateWhyCards(financial, metrics) {
    const cards = [];
    const g = (v) => `<strong class="text-green-400">${v >= 0 ? "+" : ""}${v.toFixed(1)}%</strong>`;
    const y = (v) => `<strong class="text-yellow-400">${v >= 0 ? "+" : ""}${v.toFixed(1)}%</strong>`;
    const r = (v) => `<strong class="text-red-400">${v >= 0 ? "+" : ""}${v.toFixed(1)}%</strong>`;
    // ── 매출 성장률 ──
...
// ── FinTag 생성 ───────────────────────────────────────────────
function generateTags(financial, metrics) {
    const tags = [];
    if (financial?.roe != null)
...
// ── 12개월 월별 종가 추출 ─────────────────────────────────────
function extractPriceHistory(recentPrices) {
    return [...recentPrices]
...
// ── Peer 유사도 추정 ──────────────────────────────────────────
function estimateCorrelation(baseIndutyCode, peerIndutyCode) {
    if (!baseIndutyCode || !peerIndutyCode)
        return 70;
...
function mapToFrontend(company, peers) {
...
class CompanyService {
    ...
    async getByName(name) {
        ...
        if (company && (company.financials?.length ?? 0) > 0) {
            ...
            if (peers.length < 4 && company.indutyCode) {
                ...
                const excludeStockCodes = [
                    company.stockCode ?? "",
                    ...peers.map((p) => p.stockCode ?? ""),
                ].filter(Boolean).join(",");
                const pyRes = await fetch(`${DATA_SERVICE_URL}/company/sector-peers?prefix=${prefix}&excludes=${encodeURIComponent(excludeStockCodes)}&limit=${4 - peers.length}`).catch(() => null);
                if (pyRes?.ok) {
        ...
        const res = await fetch(`${DATA_SERVICE_URL}/company?name=${encodeURIComponent(name)}`).catch(() => null);
        if (res?.ok) {
            ...
            if (pyData.peers.length < 2) {
                ...
                const extraPeers = extras.slice(0, needed).map((p) => {
                    const pf = p.financials?.[0] ?? null;
                    const pm = p.stockMetrics?.[0] ?? null;
                    const pAxes = {
                        growth: calcGrowth(pf?.revenueGrowthRate ?? null),
    ...
    async search(params) {
        ...
        const dbStockCodes = new Set(dbData.map((c) => c.stockCode).filter(Boolean));
        ...
        if (mapped.length < params.limit) {
            const pyRes = await fetch(`${DATA_SERVICE_URL}/company/search?name=${encodeURIComponent(params.query)}`).catch(() => null);
            if (pyRes?.ok) {
                const pyBody = (await pyRes.json());
    ...
    async getSimilar(params) {
        const base = await this.companyRepository.findByCorpCodeOrThrow(params.corpCode);
        if (!base.indutyCode) {
            return [];
        }
    ...
    // ── 기업 비교 분석 ─────────────────────────────────────────────
    async compare(corpCodes) {
        const companies = await this.companyRepository.findManyDetailByCorpCodes(corpCodes);
        ...
        return {
            ...
            sectorAverage: sectorAverage
                ? {
                    ...
                    avgOperatingMargin: Number(sectorAverage.avg_operating_margin) || null,
